...
class AdaptiveLossFunction(nn.Module):
  ...
  constructs free parameters (torch Parameters) that define the beta and scale
  parameters for each dimension of `x`, such that all betas are in
  (`beta_lo`, `beta_hi`) and all scales are in (`scale_lo`, Infinity).
  ...
  all images in the batch. If the user wants to fix beta or scale to be a
  constant,
  this can be done by setting beta_lo=beta_hi or scale_lo=scale_init
  respectively.
  """
  ...
  def __init__(self,
    ...
    Args:
      ...
      beta_lo: The lowest possible value for loss's beta parameters, must be
        >= 0 and a scalar. Should probably be in (0, 2).
      beta_hi: The highest possible value for loss's beta parameters, must be
        >= beta_lo and a scalar. Should probably be in (0, 2).
      beta_init: The value that the loss's beta parameters will be initialized
        to, must be in (`beta_lo`, `beta_hi`), unless `beta_lo` == `beta_hi`
        in which case this will be ignored. Defaults to (`beta_lo` +
        `beta_hi`) / 2
    ...
    if not np.isscalar(alpha_lo):
      raise ValueError('`alpha_lo` must be a scalar, but is of type {}'.format(
          type(alpha_lo)))
    if not np.isscalar(beta_lo):
      raise ValueError('`beta_lo` must be a scalar, but is of type {}'.format(
          type(beta_lo)))
    if not np.isscalar(beta_hi):
      raise ValueError('`beta_hi` must be a scalar, but is of type {}'.format(
          type(beta_hi)))
    if beta_init is not None and not np.isscalar(beta_init):
      raise ValueError(
          '`beta_init` must be None or a scalar, but is of type {}'.format(
              type(beta_init)))
    if not beta_lo >= -2:
      raise ValueError('`beta_lo` must be >= 0, but is {}'.format(beta_lo))
    
    if not beta_hi >= beta_lo:
      raise ValueError('`beta_hi` = {} must be >= `beta_lo` = {}'.format(
          beta_hi, beta_lo))
    ...
    if beta_init is not None and beta_lo != beta_hi:
      if not (beta_init > beta_lo and beta_init < beta_hi):
        raise ValueError(
            '`beta_init` = {} must be in (`beta_lo`, `beta_hi`) = ({} {})'
            .format(beta_init, beta_lo, beta_hi))
    ...
    self.distribution = distribution_skewed.DistributionSkewed()

    if beta_lo == beta_hi:
      # If the range of betas is a single item, then we just fix `beta` to be
      # a constant.
      self.fixed_beta = torch.tensor(
          beta_lo, dtype=self.float_dtype,
          device=self.device)[np.newaxis, np.newaxis].repeat(1, self.num_dims)
      self.beta = lambda: self.fixed_beta
    else:
      # Otherwise we construct a "latent" beta variable and define `beta`
      # As an affine function of a sigmoid on that latent variable, initialized
      # such that `beta` starts off as `beta_init`.
      if beta_init is None:
        beta_init = (beta_lo + beta_hi) / 2.
      latent_beta_init = util.inv_affine_sigmoid(
  ...
  def __init__(self,
    ...
    Args:
      ...
      use_students_t: If true, use the NLL of Student's T-distribution instead
        of the adaptive loss. This causes all `beta_*` inputs to be ignored.
